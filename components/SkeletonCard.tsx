...
export function SkeletonPlayerCard() {
  const colors = useColors();
  return (
    <View style={[styles.card, { backgroundColor: colors.card, borderColor: colors.border, flexDirection: "row", padding: 0, gap: 0, marginBottom: 7 }]}>
      <View style={{ flex: 1, padding: 10, gap: 8 }}>
        <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
          <Skeleton width={31} height={31} style={{ backgroundColor: colors.muted, borderRadius: 16 }} />
          <Skeleton width={100} height={11} style={{ backgroundColor: colors.muted }} />
        </View>
        <Skeleton width={90} height={9} style={{ backgroundColor: colors.muted, alignSelf: "center" }} />
        <View style={{ flexDirection: "row", justifyContent: "center", gap: 12, alignItems: "center" }}>
          <Skeleton width={30} height={20} style={{ backgroundColor: colors.muted, borderRadius: 3 }} />
          <Skeleton width={30} height={16} style={{ backgroundColor: colors.muted }} />
          <Skeleton width={30} height={20} style={{ backgroundColor: colors.muted, borderRadius: 3 }} />
        </View>
        <Skeleton width={70} height={9} style={{ backgroundColor: colors.muted, alignSelf: "center" }} />
      </View>
      <View style={{ width: "16%", borderLeftWidth: 1, borderLeftColor: colors.border, padding: 4, gap: 4, justifyContent: "center" }}>
        <Skeleton height={34} style={{ backgroundColor: colors.muted, borderRadius: 5 }} />
        <Skeleton height={34} style={{ backgroundColor: colors.muted, borderRadius: 5 }} />
      </View>
    </View>
  );
